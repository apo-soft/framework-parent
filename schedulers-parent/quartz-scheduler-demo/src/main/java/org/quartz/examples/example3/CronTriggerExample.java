...
public class CronTriggerExample {
	...
	public void run() throws Exception {
		...
		// "15 0/2 * * * ?" 第一位表示秒 延时15秒, 第二位表示分 0/2 表示 从0分开始步进为2分
		...
		// "0 0/2 8-17 * * ?" 第一位表示秒 延时为0, 第二位为分 0/2 表示每2分钟延时0分钟 第三位表示小时: 8-17
		// 表示 从8am ~~ 5pm
		...
		// 0 表示0秒执行, 0/3 表示0分开始执行步进为3分, 17-23表示时间区段小时
		...
		// job 8 will run every three seconds but only between 11am and 1pm or
		// between 5pm and 11pm
		job = newJob(SimpleJob.class).withIdentity("job8", "group1").build();

		trigger = newTrigger().withIdentity("trigger8", "group1").withSchedule(cronSchedule("0/3 * 12-14,15-17 * * ?"))
				.build();
